...
| **The desktop window, from pip** | `pip install 'python-mcp-gateway[desktop]'` then `mcp-gateway-desktop`. The same window, built with no compiler — for a machine that is not allowed a Rust toolchain. It needs a Python and a `servers.yaml`, which the bundled app does not, and **on Linux it needs a GUI backend pip does not install for you** — see [`src/mcp_gateway/window.md`](src/mcp_gateway/window.md), which has the per-platform table. |
