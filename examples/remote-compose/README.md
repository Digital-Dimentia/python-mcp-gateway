...
Compose starts all three at once, so the gateway may well dial a backend a moment before
its container is listening. That is fine now: a failed backend is retried on its own backoff
and comes up within a few seconds. Watch the log rather than intervening.
...
| …but `curl` reaches that port fine from the box | A startup race, and it resolves itself: the gateway retries a failed backend on a backoff that starts at once and caps at a minute. Wait a few seconds. `gateway__restart_backend` asks for an attempt now; `SIGHUP` will *not* help, because reload restarts only backends whose config changed. |
